...
describe('CheckSchedulerService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(Monitor), useValue: mockMonitorRepository },
      ],
    }).compile();
  ...
  describe('onModuleInit', () => {
    it('should initialize priority queue if it does not exist and lock is acquired', async () => {
      ...
      const expectedNextRun = new Date('2024-01-01T00:00:00Z').getTime() + 60000;
      expect(mockPriorityQueue.addItem).toHaveBeenCalledWith('monitors', expectedNextRun, monitor.id);
    ...
    it('should wait for another instance if lock is not acquired', async () => {
        // Arrange
        mockPriorityQueue.checkDataExists!
            .mockResolvedValueOnce(false) // 1. Initial check (needs init)
  ...
  describe('processDueMonitors', () => {
    it('should process due monitors, emit Kafka events, and reschedule them', async () => {
      ...
      mockKafkaProducerService.emitCheckExecutionRequested!.mockResolvedValue(true);

      await (service as any).processDueMonitors();

      expect(priorityQueue.getDueItems).toHaveBeenCalledWith(100);
      expect(priorityQueue.removeItem).toHaveBeenCalledWith('monitors', monitorId);
      expect(kafkaProducerService.emitCheckExecutionRequested).toHaveBeenCalledWith(expect.objectContaining({
        checkId: monitorId
      }));
      
      const expectedNextRun = new Date('2024-01-01T00:00:00Z').getTime() + 60000;
      expect(priorityQueue.addItem).toHaveBeenCalledWith('monitors', expectedNextRun, monitorId);
    });
  ...
  describe('getMonitor', () => {
    ...
    it('should get monitor from repository if not in cache', async () => {
      ...
      expect(cacheService.getMonitor).toHaveBeenCalledWith(monitorId);
      expect(monitorRepository.findOne).toHaveBeenCalledWith({ where: { id: monitorId } });
    ...
    it('should throw an error if monitor not found', async () => {
        ...
        await expect((service as any).getMonitor(monitorId))
            .rejects.toThrow(`Monitor with ID ${monitorId} not found in database`);
      });
  });
});