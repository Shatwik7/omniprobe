import { Controller, Get, Post, Body, Patch, Param, Delete, UseGuards, Query, UnauthorizedException, NotAcceptableException, NotFoundException } from '@nestjs/common';
...
export class MetricsController {
  ...
  async create(@Body() createMetricDto: CreateMetricDto) {
    await this.pollingService.publishUpdate(createMetricDto.monitorId,createMetricDto);
    return this.metricsService.create(createMetricDto);
  }

  @Get()
  @UseGuards(TeamMemberGuard)
  async findAll(@Param('monitorId') monitorId: string, @Param('projectId') projectId: string, @Query('beginDate') beginDate: Date, @Query('endDate') endDate: Date, @Query('region') region: string) {
    if (await this.metricsService.checkMonitorInProject(monitorId, projectId)) {
      throw new NotAcceptableException('Monitor does not belong to the specified project');
    }
  ...
  @Get('poll')
  @UseGuards(TeamMemberGuard)
  async poll(@Param('monitorId') monitorId: string, @Param('projectId') projectId: string) {
    if (await this.metricsService.checkMonitorInProject(monitorId, projectId)) {
      throw new NotAcceptableException('Monitor does not belong to the specified project');
    }
